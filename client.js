...
<script>
    ...
    const DominoGame = () => {
        ...
        const [playerStats, setPlayerStats] = useState(null); // Oyuncu istatistikleri için yeni state
        ...
        const handleServerMessage = (data) => {
            switch (data.type) {
                ...
                case 'matchFound':
                    ...
                    break;
                case 'statsUpdate': // Yeni: İstatistik güncelleme
                    setPlayerStats(data.player);
                    console.log('📊 Oyuncu istatistikleri güncellendi:', data.player);
                    break;
                case 'playerDisconnected':
                    ...
                    setScreen('lobby');
                    setGameState(null);
                    setRoomCode(''); // Oda kodunu temizle
                    break;
                case 'gameEnd':
        ...
        const startRankedMatch = () => {
            ...
            setSearching(true);
            sendMessage({ type: 'findMatch', playerName, ...playerStats }); // İstatistikleri de gönder
        ...
        if (screen === 'lobby') {
            return React.createElement('div', { 
                ...
                React.createElement('div', { className: 'relative z-10 max-w-2xl w-full' },
                    ...
                    playerStats && React.createElement('div', { className: 'text-center mb-4 text-white font-semibold' },
                        `ELO: ${playerStats.elo} | Seviye: ${playerStats.level} | G: ${playerStats.wins} M: ${playerStats.losses} B: ${playerStats.draws}`
                    ),
        ...
        if (screen === 'game' && gameState) {
            ...
            return React.createElement('div', { className: 'min-h-screen bg-gradient-to-br from-green-800 via-green-700 to-emerald-900 p-4 relative overflow-hidden' },
                ...
                React.createElement('div', { className: 'max-w-7xl mx-auto relative z-10' },
                    ...
                    React.createElement('button', {
                        onClick: leaveGame,
                        className: 'absolute top-2 left-2 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-lg transition-transform transform hover:scale-105'
                    }, '🚪 Oyundan Çık'),
