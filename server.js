// Dosya Adı: server.js (BOMBALI HAFIZA OYUNU V4 - EŞ ZAMANLI)
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ username, roomCode }) => {
        ...
        io.to(code).emit('gameStart', { players, hostBombs: room.hostBombs, guestBombs: room.guestBombs, boardSize: BOARD_SIZE, initialLives: DEFAULT_LIVES });
    });
    ...
    socket.on('MOVE', (data) => {
        ...
        // KRİTİK: Bu oyuncu Host mu Guest mi?
        const isHostPlayer = socket.id === room.hostId;
        const playerName = isHostPlayer ? room.hostUsername : room.guestUsername;

        if (room.board[cardIndex]) { socket.emit('infoMessage', { message: 'Kart zaten açık.', isError: true }); return; }
