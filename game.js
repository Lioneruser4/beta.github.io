...
function onSocketOpen() {
    ...
    // Eğer oyun başlamışsa, oyun durumunu yeniden iste
    if (gameState.gameStarted && gameState.roomCode && gameState.currentPlayerId) {
        console.log('🔄 Oyun durumu yeniden isteniyor...');
...
function handleGameUpdate(data) {
    ...
    gameState.myColor = data.myColor || gameState.myColor;
    gameState.isMyTurn = gameState.currentPlayerId === gameState.currentPlayerId;
    
    // Oyun tahtasını güncelle
    if (gameState.gameStarted) {
        updateGameUI(gameState);
    } else if (data.gameState) {
        // Eğer oyun başlamadıysa ve gameState verisi geldiyse
        handleGameStart(data);
    }
    ...
    if (gameState.isMyTurn) {
        ...
        turnText.className = 'text-green-500 font-bold';
    } else {
        turnText.textContent = 'Rakibiniz Oynuyor...';
        turnText.className = 'text-yellow-500';
    }
}
